...
kernel_prep: prep
	...
	-rm $(SRC)/linux && ln -s $(SRC)/linux-3.2.2 $(SRC)/linux
...
bld_binutil1: kernel_prep 
	...
	@(if [ ! -e $(BLD)/.build_binutils_1 ] ; then \
		cd $(bld_binutils_dir); \
		$(ROOT)/src/binutils-2.22/configure --target=$(TARGET) --prefix=$(bld_stage1) \
		--disable-nls --with-sysroot=$(PREFIX) --enable-shared --disable-multilib && \
		make all -j$(NR_CPU)&& make install && \
		cp -v $(SRC)/binutils-2.22/include/libiberty.h $(PREFIX)/usr/include &&\
...
bld_gmp: kernel_prep
	...
	@(if [ ! -e $(BLD)/.build_gmp ]; then \
		cd $(bld_gmp_dir);\
		$(ROOT)/src/gmp-5.0.2/configure --prefix=$(bld_stage1) && \
		make install; \
	)
...
bld_mpc: bld_mpfr
	...
	@(if [ ! -e $(BLD)/.build_mpc ]; then \
		cd $(bld_mpc_dir);\
		$(ROOT)/src/mpc-0.9/configure LD_FLAGS="-Wl,-rpath=$(bld_stage1)/lib" --prefix=$(bld_stage1) \
...
bld_mpfr: bld_gmp
	...
	@(if [ ! -e $(BLD)/.build_mpfr ]; then \
		cd $(bld_mpfr_dir);\
		$(ROOT)/src/mpfr-3.1.0/configure LD_FLAGS="-Wl,-rpath=$(bld_stage1)/lib" --prefix=$(bld_stage1) \
...
bld_gcc1: bld_binutil1 bld_mpc
	...
	@(if [ ! -e $(BLD)/.build_gcc_1 ] ; then \
		export PATH=$(PATH);\
		cd $(bld_gcc_dir); \
		$(ROOT)/src/gcc-4.6.2/configure AR=ar LDFLAGS="-Wl,-rpath=$(PREFIX)/tmp/lib" \
